    ...
    if self.elemento.is_a? ElementoConCosto
      ...
      if self.elemento.formula.match(/\+/)
        ...
        operandos = operandos.collect { |o|
          if o.match(/\$/)
            self.vigilador.datos.select{|d| d.read_attribute(:elemento_id) == o.gsub("$", "").to_i}[0].costo
          else
            o.to_i
          end
        }
        ...
        return self.vigilador.datos.select{|d| d.read_attribute(:elemento_id) == elemento_id.to_i}[0].valor * k.to_f
      end
